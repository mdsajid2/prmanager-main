...
Module.prototype.require = function(...args) {
  try {
    ...
    const serverNodeModules = require('path').resolve(__dirname, '../server/node_modules', args[0]);
    return originalRequire.apply(this, [serverNodeModules]);
  }
};
...
class MigrationManager {
  ...
  async getMigrationFiles() {
    ...
    const files = fs.readdirSync(migrationDir)
      .filter(file => file.endsWith('.sql') && file !== 'migration-manager.js')
      .sort();
